...
const { OUTPUT_I18N_DIR, getTemplate, getNameCrowdinFile, checkEnvKey, getProjectName } = require('../config');

const PROJECT_NAME = getProjectName();
const CROWDIN_NAME_FILE = getNameCrowdinFile();
const TEMPLATE_FILE_PATH = getTemplate(true);
const TEMPLATE_FILE = path.basename(TEMPLATE_FILE_PATH);
...
const getFileNameLang = (lang) => {
    const file = path.basename(CROWDIN_NAME_FILE, '.pot');
    return `${file}-${lang}.po`;
};
...
async function udpate(spinner) {
    ...
    form.append(`files[/${CROWDIN_NAME_FILE}]`, fs.createReadStream(TEMPLATE_FILE_PATH), {
