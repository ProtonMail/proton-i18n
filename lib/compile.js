...
async function run({ file, lang }) {
    ...
    const jsonFile = JSON.parse(await fs.readFile(output));
    ...
    const [language] = jsonFile.headers.language.split('_');
    jsonFile.headers.language = language;

    return fs.writeFile(output, JSON.stringify(jsonFile));
}
